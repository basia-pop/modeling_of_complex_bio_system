...
```{r}
#pdf("Poplawska_problem1.pdf",height=7,width=7)
heatmap.2(edata,
          main = "Poplawska_problem1", 
          notecol="black",
          density.info="none",  
          trace="none",         
          margins =c(12,9),    
...
```{r}
#pdf("Poplawska_problem2.pdf",height=7,width=7)
...
```{r}
#pdf("Poplawska_problem3.pdf",height=7,width=7)
par(mfrow=c(1,1))
left_2 <- data.table(u1 = svd.out$u[, 1], u2 = svd.out$u[, 2])
plot(left_2, pch = 20, main = "Top 2 left singular vectors")
...
```{r}
#pdf("Poplawska_problem4.pdf",height=7,width=7)
top_5 <- data.table( svd.out$u[, 1:5]) 
top_5[, Index := .I]

top_5_long <- melt(top_5, id.vars = "Index", variable.name = "left_singular_vectors", value.name = "left_singular_vectors_values")


ggplot(top_5_long, aes(x = left_singular_vectors, y = left_singular_vectors_values)) + geom_violin(draw_quantiles = c(0.25, 0.5, 0.75)) + labs(title = "Top 5 Left Singular Vectors")
...
```{r}
#pdf("Poplawska_problem5.pdf",height=7,width=10)

set.seed(4) 
clustering <- kmeans(edata, centers=5)
cluster_nr <- clustering$cluster

tsne_out1 <- Rtsne(edata,pca=FALSE,perplexity=60)
tsne_out1 = data.table(tsne_out1$Y, cluster_nr)

ggplot(tsne_out1) + geom_point(aes(x=V1, y=V2, color = as.factor(cluster_nr)))
