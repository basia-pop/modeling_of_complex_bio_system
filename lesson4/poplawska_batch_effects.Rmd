...
edata_ordered <- edata[, order(pheno$batch)]
combat_edata_ordered <- combat_edata[, order(pheno$batch)]

png("poplawska_problem2_before.png", height=700, width=900)
...
png("poplawska_problem2_after.png", height=700, width=900)
...
cor_cancer<- cor(edata[,order(pheno$cancer)],method='pearson')
cor_batch<- cor(edata[,order(pheno$batch)],method='pearson')
cor_outcome<- cor(edata[,order(pheno$outcome)],method='pearson')
collumns <- row.names(pheno)

pdf("poplawska_problem3.pdf", height=7, width=9)
heatmap.2(cor_cancer,
          main = "Corelation cancer vs. normal",
          notecol="black",
          density.info="none",
          trace="none",
          margins=c(12,9),
          ...
          Rowv = FALSE,
          Colv = FALSE,
          labRow = paste(pheno$cancer[order(pheno$cancer)],collumns(cor_cancer) , sep = "_"),
          labCol = paste(sort(pheno$cancer),collumns(cor_cancer), sep = "_"))

heatmap.2(cor_batch,
          main = "Corelation batch numbers",
          notecol="black",
          density.info="none",
          trace="none",
          margins=c(12,9),
          ...
          Rowv = FALSE,
          Colv = FALSE,
          labRow = paste(sort(pheno$batch), collumns(cor_batch), sep = "_"),
          labCol = paste(sort(pheno$batch), collumns(cor_batch) ,sep = "_"))

heatmap.2(cor_batch,
          main = "Corelation outcome",
          notecol="black",
          density.info="none",
          trace="none",
          margins=c(12,9),
          ...
          Rowv = FALSE,
          Colv = FALSE,
          labRow = paste(sort(pheno$outcome), collumns(cor_batch), sep = "_"),
          labCol = paste(sort(pheno$outcome), collumns(cor_batch), sep = "_"))
dev.off()
```
...
```{r}
pdf("poplawska_problem4.pdf", height=7, width=9)

edata<- as.matrix(exprs(bottomly.eset))
edata <- edata[rowMeans(edata) > 10, ]
edata <- log2(as.matrix(edata) + 1)
pdata <- pData(bottomly.eset)
mod = lm(t(edata) ~ as.factor(pdata$strain) + as.factor(pdata$experiment.number))
mod_tidy <- tidy(mod)
combat_edata = ComBat(dat=edata, batch=pdata$experiment.number, mod=model.matrix(~1, data=pdata), par.prior=TRUE, prior.plots=TRUE)
modcombat = lm(t(combat_edata) ~ as.factor(pdata$strain))
modcombat_tidy <- tidy(modcombat)

ggplot(mod_tidy %>% filter(term == "as.factor(pdata$strain)DBA/2J")) + geom_histogram(aes(x=p.value), bins = 100, fill="darkorange") +ggtitle('uncorrected P-value')
ggplot(mod_tidy, aes(estimate, term)) +
     geom_point() +
     geom_vline(xintercept = 0) + ggtitle('uncorrected coefficients plot')

ggplot(modcombat_tidy %>% filter(term == "as.factor(pdata$strain)DBA/2J")) + geom_histogram(aes(x=p.value), bins = 100, fill="darkorange") + ggtitle('corrected P-value')
ggplot(modcombat_tidy, aes(estimate, term)) +
     geom_point() +
     geom_vline(xintercept = 0) +ggtitle('corrected coefficients plot')
...
ggplot(est_compare, aes(x=LinearModel, y=ComBat)) +
     geom_point(col="darkgrey", alpha=.5, size=.5) + geom_abline(intercept=0, slope=1, col="darkred") + geom_smooth(method = "lm", se = TRUE)  + theme_bw() +ggtitle('Comparison uncorrected vs. corrected')
dev.off()
