...
kmeans.result <- kmeans(Embeddings(pbmc, "pca")[, 1:10], centers = 10)

pbmc$kmeans_clusters <- factor(kmeans.result$cluster)
cluster_counts <- table(pbmc$kmeans_clusters)
cluster_percents <- round(100 * cluster_counts / sum(cluster_counts), 1)

plot_labels <- paste0(names(cluster_counts), " (", cluster_percents, "%)")

pbmc$kmeans_clusters_percent <- factor(pbmc$kmeans_clusters, levels = names(cluster_counts),
                                       labels = plot_labels)

pdf("Poplawska_problem1.pdf")
DimPlot(pbmc, reduction = "tsne", label = TRUE, pt.size = 0.2,group.by = "kmeans_clusters_percent") +
  ggtitle("t-SNE with 10 clusters + %")
...
Idents(pbmc) <- pbmc$kmeans_clusters
subcluster_elements <- setNames(rep(NA, ncol(pbmc)), colnames(pbmc))


for (cluster_number in levels(pbmc$kmeans_clusters)) {
  
  cells <- WhichCells(pbmc, idents = cluster_number)
  pca_subset <- Embeddings(pbmc, reduction = "pca")[cells, 1:10]
  
  best_k <- 2
  best_s <- -1
  
  for (k in 2:6) {
    km <- kmeans(pca_subset, centers = k, nstart = 10)
    s <- silhouette(km$cluster, dist(pca_subset))
    avg_s <- mean(s[, 3])
  ...
  result_kmean <- kmeans(pca_subset, centers = best_k, nstart = 10)
  lables <- paste0("Cluster", cluster_number, "-", result_kmean$cluster)
  subcluster_elements[cells] <- lables
}


pbmc$subcluster <- subcluster_elements

pdf("Poplawska_problem2.pdf")
DimPlot(pbmc, reduction = "tsne", group.by = "subcluster", label = TRUE, pt.size = 0.3) +
  ggtitle("t-SNE for all clusters with their subclusters")

Idents(pbmc) <- "subcluster"
main_clusters <- levels(pbmc$main_cluster)

for (cl in main_clusters) {
  pbmc_subset <- subset(pbmc, subset = main_cluster == cl)
  print(
    DimPlot(pbmc_subset, reduction = "tsne", label = TRUE, pt.size = 0.3) +
      ggtitle(paste("t-SNE for Cluster", cl))
  )
}
dev.off()
